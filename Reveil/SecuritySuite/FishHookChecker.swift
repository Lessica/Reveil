...
/*
  ...
  */
    ...
    private func readUleb128(ptr: inout UnsafeMutablePointer<UInt8>, end: UnsafeMutablePointer<UInt8>) -> UInt64 {
        ...
        repeat {
            if ptr == end {
                assertionFailure("malformed uleb128")
            }
            let slice = UInt64(ptr.pointee & 0x7F)
            if bit > 63 {
                assertionFailure("uleb128 too big for uint64")
            } else {
                result |= (slice << bit)
                bit += 7
            }
    ...
    private func readSleb128(ptr: inout UnsafeMutablePointer<UInt8>, end: UnsafeMutablePointer<UInt8>) -> Int64 {
        ...
        repeat {
            if ptr == end {
                assertionFailure("malformed sleb128")
            }
            byte = ptr.pointee
    ...
    enum SymbolFound {
        private static let BindTypeThreadedRebase = 102
        ...
        private static func lookLazyBindSymbol(_ symbol: String, symbolAddr: inout UnsafeMutableRawPointer?, lazyBindInfoCmd: UnsafeMutablePointer<UInt8>, lazyBindInfoSize: Int, allLoadDylds: [String]) -> Bool {
            ...
            Label: while ptr < lazyBindingInfoEnd {
                ...
                default:
                    assertionFailure("bad lazy bind opcode")
                    return false
                }
            }
                ...
                case BIND_OPCODE_THREADED:
                    ...
                    case BIND_SUBOPCODE_THREADED_APPLY:
                        if foundSymbol {
                            ...
                            assertionFailure("maybe bind_type is BIND_TYPE_THREADED_REBASE, don't handle")
                            return false
                        }
                        continue Label
                    default:
                        assertionFailure("bad bind subopcode")
                        return false
                    }
                default:
                    assertionFailure("bad bind opcode")
                    return false
                }
            }
        ...
        @inline(__always)
        private static func _lookExportedSymbol(_ symbol: String, image: UnsafePointer<mach_header>, imageSlide slide: Int) -> UnsafeMutableRawPointer? {
            // target cmd
            var linkeditCmd: UnsafeMutablePointer<segment_command_64>!
            var dyldInfoCmd: UnsafeMutablePointer<dyld_info_command>!
        ...
        // ExportSymbol
        @inline(__always)
        private static func lookExportedSymbolByTrieWalk(targetSymbol: String, start: UnsafeMutablePointer<UInt8>, end: UnsafeMutablePointer<UInt8>, currentLocation location: UnsafeMutablePointer<UInt8>, currentSymbol: String) -> UnsafeMutablePointer<UInt8>? {
            var ptr = location
