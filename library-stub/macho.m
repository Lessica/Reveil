...
void machoEnumerateLoadCommands(FILE *machoFile, uint32_t archOffset, void (^enumerateBlock)(struct load_command cmd, uint32_t cmdOffset))
{
    ...
    uint32_t offset = 0;
    NSLog(@"[machoEnumerateLoadCommands] About to enumerate over %u load commands (total size: 0x%X)", nCmds, sizeOfCmds);
    ...
    NSLog(@"[machoEnumerateLoadCommands] Finished enumerating over %u load commands (total size: 0x%X)", nCmds, sizeOfCmds);
}
                    ...
                    if (nextFile) {
                        ...
                        if (nextFileIsMacho) {
                            ...
                            if (nextBestArchCandidate >= 0) {
                                _machoEnumerateDependencies(nextFile, (uint32_t)nextBestArchCandidate, resolvedPath, sourceExecutablePath, enumeratedCache, enumerateBlock);
                            }
                            else {
                                NSLog(@"[_machoEnumerateDependencies] Failed to find best arch of dependency %s", resolvedPath.UTF8String);
                            }
                        }
                        else {
                            NSLog(@"[_machoEnumerateDependencies] Dependency %s does not seem to be a macho", resolvedPath.UTF8String);
                        }
                        fclose(nextFile);
                    }
                    else {
                        NSLog(@"[_machoEnumerateDependencies] Dependency %s does not seem to exist, maybe path resolving failed?", resolvedPath.UTF8String);
                    }
                }
                else {
                    if (![[NSFileManager defaultManager] fileExistsAtPath:resolvedPath]) {
                        NSLog(@"[_machoEnumerateDependencies] Skipped dependency %s, non existant", resolvedPath.UTF8String);
                    }
                    else {
                        NSLog(@"[_machoEnumerateDependencies] Skipped dependency %s, already cached", resolvedPath.UTF8String);
                    }
                }
            }
            else {
                NSLog(@"[_machoEnumerateDependencies] Skipped dependency %s, in dyld_shared_cache", imagePath.UTF8String);
            }
        }
    });
}
